...
//        echo '$pair=' . $pair . ' $pair_1=' . $pair_1 . ' - $pair_2=' . $pair_2 . PHP_EOL;
        ...
        $pairs[$pair_1] = gmp_add($pairs[$pair_1], $count);
        if (!isset($pairs[$pair_2])) {
            $pairs[$pair_2] = 0;
        }
        $pairs[$pair_2] = gmp_add($pairs[$pair_2], $count);

        $pairs[$pair] = gmp_sub($pairs[$pair], $count);

    }

    $pairs = array_filter($pairs, function($count) {
        return gmp_cmp($count, 0) === 1;
    });
...
foreach ($pairs as $pair => $count) {
    ...
    $stats[$letter_a] = gmp_add($stats[$letter_a], $count);
    $stats[$letter_b] = gmp_add($stats[$letter_b], $count);
...
echo '$step=' . ($step - 1) . ' ' . '$length=' . $length . ' $diff=' . gmp_sub($first, $end) . PHP_EOL;