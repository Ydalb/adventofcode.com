...
//        echo '$pair=' . $pair . ' $pair_1=' . $pair_1 . ' - $pair_2=' . $pair_2 . PHP_EOL;
        ...
        $pairs[$pair_1] = bcadd($pairs[$pair_1], $count);
        if (!isset($pairs[$pair_2])) {
            $pairs[$pair_2] = 0;
        }
        $pairs[$pair_2] = bcadd($pairs[$pair_2], $count);

        $pairs[$pair] = bcsub($pairs[$pair], $count);

    }

    $pairs = array_filter($pairs, function($count) {
        return bccomp($count, 0) === 1;
    });
...
foreach ($pairs as $pair => $count) {
    ...
    $stats[$letter_a] = bcadd($stats[$letter_a], $count);
    $stats[$letter_b] = bcadd($stats[$letter_b], $count);
...
echo '$step=' . ($step - 1) . ' ' . '$length=' . $length . ' $diff=' . bcsub($first, $end) . PHP_EOL;